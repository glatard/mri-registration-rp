...
We used the \textit{BMB\_1} site from the \textit{CoRR} dataset~\cite{Zuo2014-ub} ($N=50$; mean age: $30.8$ [19.9--59.7]; $52\%$ female). Anatomical $T1$-weighted volumes were skull-stripped via the antsBrainExtraction.sh script~\cite{Avants2020-xx,ANTs2023-du} and quality-controlled against the \textit{tpl-MNI152NLin2009cAsym} template~\cite{Ciric2022-mb}. Experiments were executed on the \textit{Rorqual} cluster of the Digital Research Alliance of Canada using containerized Apptainer images with architecture-specific instructions disabled to ensure reproducibility across CPU microarchitectures.
