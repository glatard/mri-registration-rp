...
#########################
export SIF_IMG=${SIF_DIR}/ants-baseline.simg
job1=$(sbatch --parsable --array=1-${NUM_SUBJECTS} ${SLURM_OPTS} ./code/antsBrainExtraction.sbatch)

################################
# ANTs Registration (Binary64) #
################################
...
sbatch --job-name=${EXPERIMENT_NAME} --dependency=afterok:$job1 --array=1-${NUM_SUBJECTS} ${SLURM_OPTS} ./code/space-search/antsRegistration-0000.sbatch
